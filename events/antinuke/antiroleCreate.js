  ...
  let set = await client.db.get(`setup_${role.guild.id}`)
   ...
   let del = await logs.target.delete().catch(() => { });
              ...
              if (punish == `ban`) {             
                ...
                let x = await role.guild.members.ban(logs.executor.id, {reason: `Kranton Antinuke | Role Created Not Whitelisted`}).catch(() => {})
              ...
              if (punish == `kick`) {
                ...
                let ch = await role.guild.channels.fetch(logch).catch(() => {})
                if (!ch || ch === null) return;
                let x = await role.guild.members.kick(logs.executor.id, {reason: `Kranton Antinuke | Role Created Not Whitelisted`}).catch(() => {})
              ...
              if (punish == `mute`) {
                ...
                let x = await mem.timeout(dur, {reason: `Kranton Antinuke | Role Created Not Whitelisted`}).catch(() => {})
