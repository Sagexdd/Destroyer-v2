  ...
  let set = await client.db.get(`setup_${channel.guild.id}`)
   ...
   let del = await logs.target.delete().catch(() => { });
              ...
              if (punish == `ban`) {             
                ...
                let x = await channel.guild.members.ban(logs.executor.id, {reason: `Kranton Antinuke | Channel Created Not Whitelisted`}).catch(() => {})
              ...
              if (punish == `kick`) {
                ...
                let ch = await channel.guild.channels.fetch(logch).catch(() => {})
                if (!ch || ch === null) return;
                let x = await channel.guild.members.kick(logs.executor.id, {reason: `Kranton Antinuke | Channel Created Not Whitelisted`}).catch(() => {})
              ...
              if (punish == `mute`) {
                ...
                let x = await mem.timeout(dur, {reason: `Kranton Antinuke | Channel Created Not Whitelisted`}).catch(() => {})
